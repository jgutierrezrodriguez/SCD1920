...
public class Cliente implements Constantes,Runnable {
    ...
    private void crearPeticiones(int totalPeticiones) throws InterruptedException {
        ...
        for (int i = 0; i < totalPeticiones; i++ ) {
            ...
            // Calculamos el retardo proporcional a la prioridad
            retardoPeticion = new Date();
            tiempoRetardo = TIEMPO_MINIMO + generador.nextInt(VARIACION_TIEMPO);
            tiempoRetardo = tiempoRetardo * prioridadPeticion.getPenalizacion();
            retardoPeticion.setTime(inicioPeticion.getTime() + 
                    TimeUnit.MILLISECONDS.convert(tiempoRetardo, TimeUnit.SECONDS));
            
            peticion = new Peticion(prioridadPeticion,idCliente,listaPedidos,
                                                inicioPeticion, retardoPeticion);
